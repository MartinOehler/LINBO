  ...
  for (drive = 0x80; drive < 0x88; drive++)
    {
      ...
      // read mbr of current drive
      rawread( drive, 0, 0, 512, mbr );
                  ...
                  if (open_device ())
                    {
                      ...
                      // see whether we found our target file - in the LINBO special case a menu.lst
                      if (grub_open (filename))
                        {
                          ...
                          /* if we found our menu.lst, the following things are to do:
                             - check whether the autoboot flag is set (1E0)
                             - check whether the drive equals the preselected autoboot drive (1E1)
                             - check whether the slice equals the preselected slice (1E2)
                          */
                        
                          grub_printf("mbr[432]=%d\n",((int)mbr[432]));
                          grub_printf("mbr[432]h=%X\n",mbr[432]);
                        
                          grub_printf("mbr[433]=%d\n",((int)mbr[433]));
                          grub_printf("mbr[433]h=%X\n",mbr[433]);
                        
                          grub_printf("mbr[434]=%d\n",((int)mbr[434]));
                          grub_printf("mbr[434]h=%X\n",mbr[434]);
                        
                          if( ((int)mbr[432]) ) {
                            grub_printf("MBR ändern...\n");
                            mbr[432] = 0;
                            grub_printf("mbr[432]d=%d\n",((int)mbr[432]));
                            grub_printf("mbr[432]h=%X\n",mbr[432]);
                            rawwrite( drive, 0, mbr );
                          
                            grub_printf("Buffer erfolgreich geschrieben?\n");
                            grub_printf(" Pruefe Ergebnis:\n");
                            rawread( drive, 0, 0, 512, mbr );
                            grub_printf("mbr[432]=%d\n",((int)mbr[432]));
                            grub_printf("mbr[432]h=%X\n",mbr[432]);
                          ...
                          grub_printf( "got_file = %d", got_file);
...
real_root_func (char *arg, int attempt_mount)
{
  ...
  /*      if (open_partition ())
          ...
          grub_printf("after attempt_mount");
  */
  /* Clear ERRNUM.  */
  errnum = 0;
  saved_partition = current_partition;

...
#if POSIXACLS & !defined(HAVE_CONFIG_H)
...
int ntfs_build_mapping(struct MAPPING *[], const char*);
void newblock(s32);
...
uid_t relay_find_user(const struct MAPPING *mapping, const SID *usid)
   {
   int uid;
...
gid_t relay_find_group(const struct MAPPING *mapping, const SID *gsid)
   {
   int gid;
...
int basicread(void *fileid, char *buf, size_t size, off_t pos)
{
...
int ntfs_build_mapping(struct MAPPING *mapping[], const char *usermap_path)
{
#ifdef WIN32
...
#if POSIXACLS
	ntfs_build_mapping(context.mapping, (const char*)NULL);
#endif
			/* first check samples */
	...
	if (!stat(fullname,&st)) {
		...
		if (attrsz > 0) {
			...
			if (getxattr(fullname,"system.ntfs_attrib",&attrib,4) != 4) {
				...
				printf("Windows attrib : 0x%x\n",(int)attrib);
			if (ntfs_valid_descr(attr,attrsz)) {
				mapped = !ntfs_build_mapping(context.mapping,fullname);
#if POSIXACLS
				if (mapped) {
					pxdesc = linux_permissions_posix(attr,isdir);
					if (pxdesc)
...
#if POSIXACLS
						if (ntfs_build_mapping(context.mapping,filename)) {
			...
			if (*p) {
				...
				if (size) {
					...
					if (filename) {
						makeutf16(filename,argv[xarg + 1]);
#if POSIXACLS
						if (ntfs_build_mapping(&context,filename)) {
							printf("*** Could not get user mapping data\n");
							warnings++;
						}
#endif
